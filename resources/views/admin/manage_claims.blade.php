<x-admin-layout :title="'Manage Claims'">
    <style>
        .page-title {
            font-size: 28px;
            font-weight: bold;
        ...
        <div class="tabs-container">
            <button class="tab-button active" onclick="filterByStatus('all')">All claims ({{ $allClaimsCount }})</button>
            ...
            <table class="table">
                ...
                <tbody id="claimsTableBody">
                    @forelse($claims as $claim)
                        <tr class="claim-row" data-status="{{ $claim->status }}" data-search="{{ strtolower($claim->item->name . ' ' . $claim->user->name) }}">
                            <td>{{ $claim->item->name }}</td>
                            <td>{{ $claim->item->user->name }}</td>
                            <td>{{ $claim->user->name }}</td>
                            <td>{{ $claim->user->email }}</td>
                            <td>{{ $claim->date_claimed->format('M d, Y') }}</td>
                            <td>
                            ...
                            <td>
                                <button class="view-button" onclick="openClaimModal({{ $claim->id }})">View</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                    ...
                    modalBody.innerHTML = `
                        <div class="modal-body-columns">
                            ...
                            <div class="modal-column">
                                ...
                                <div class="image-container">
                                    ${proofImageUrl ? `
                                        <img src="${(proofImageUrl.startsWith && (proofImageUrl.startsWith('http') || proofImageUrl.startsWith('data:'))) ? proofImageUrl : '/storage/' + proofImageUrl}" alt="Proof of ownership" />
                                    ` : `
